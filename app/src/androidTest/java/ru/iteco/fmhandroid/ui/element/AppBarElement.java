...
public class AppBarElement {
    ...
    //Кнопка Меню
    public static ViewInteraction MenuButton = onView(Matchers.allOf(withId(R.id.main_menu_image_button),
                    withContentDescription("Main menu")));

    //Кнопка выхода из приложения
    public static ViewInteraction Exit = onView(withId(android.R.id.title));

    //Кнопка Развернуть цитату
    public static ViewInteraction ExpandTheQuoteButton = onView(
            Matchers.allOf(withId(R.id.our_mission_item_list_recycler_view)));

    //Кнопка Свернуть цитату
    public static ViewInteraction RollUpTheQuoteButton = onView(
            Matchers.allOf(withId(R.id.our_mission_item_list_recycler_view)));

    //Строка Main в выпадающем списке
    public static ViewInteraction LineMain = onView(
            Matchers.allOf(withId(android.R.id.title), withText("Main")));

    //Строка News в выпадающем списке
    public static ViewInteraction LineNews = onView(Matchers.allOf(withId(android.R.id.title),
            withText("News")));

    //Строка About в выпадающем списке
    public static ViewInteraction LineAbout = onView(Matchers.allOf(withId(android.R.id.title),
            withText("About")));

    //Надпись Love is all
    public static ViewInteraction LoveIsAllTitle = onView(Matchers.allOf(
            withId(R.id.our_mission_title_text_view), withText("Love is all"),
                    withParent(withParent(IsInstanceOf.<View>instanceOf(android.widget.LinearLayout.class)))));

    //Значек бабочки
    public static ViewInteraction ButterflyBadge = onView(Matchers.allOf(
            withId(R.id.our_mission_image_button),withContentDescription("Our Mission")));

    //Значек человечка на панеле App bar
    public static ViewInteraction ManButton = onView(Matchers.allOf(withId(R.id.authorization_image_button)));

    //Цитата
    public static ViewInteraction Quote = onView(
